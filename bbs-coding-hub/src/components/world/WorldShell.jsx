...
export function WorldShell({ title, children }) {
  ...
  return (
    ...
    >
      ...
      <header className="w-full flex justify-between items-center p-6 md:px-12 md:py-8 border-b border-antique-gold/10 shrink-0 z-50 bg-space-dark/50 backdrop-blur-md relative">
        <div className="flex flex-col">
          <h1 className="font-sans text-[10px] tracking-[0.3em] text-antique-gold/70">
            BBS CODING CLUB / <span className="text-ivory font-medium">{title.toUpperCase()}</span>
          </h1>
        </div>
        <button 
          onClick={exitWorld}
          className="group flex items-center gap-3 font-sans text-[10px] tracking-[0.2em] text-parchment/70 hover:text-ivory transition-colors duration-300 cursor-pointer"
        >
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden relative z-10 w-full h-full scroll-smooth scrollbar-thin scrollbar-thumb-antique-gold/30 scrollbar-track-transparent">
        {children}
      </main>
    </div>
  );
