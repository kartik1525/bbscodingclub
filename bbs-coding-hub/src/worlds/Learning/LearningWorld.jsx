                ...
                <div className="flex flex-wrap items-center gap-2 pb-2">
                  {path.nodes.map((node, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <div className="font-sans text-[9px] tracking-[0.1em] text-antique-gold/80 px-3 py-1.5 border border-antique-gold/30 rounded bg-space-dark/80 group-hover:bg-space-dark hover:scale-105 hover:border-antique-gold transition-all duration-300 cursor-pointer shadow-none hover:shadow-[0_0_10px_rgba(166,124,66,0.2)]">
                        {node}
                      </div>
                      {i < path.nodes.length - 1 && <span className="text-antique-gold/30 text-[8px]">→</span>}
                    </div>
                  ))}
                </div>
              </div>
